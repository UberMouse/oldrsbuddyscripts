...
public class BulwarkBeast extends Plugin
{
    ...
    public boolean isValid() {
        return UberNpcs.getNearest(".*wark beast") != null;
    }
    ...
    private State getState() {
        ...
        else if (UberEquipment.contains("pickaxe", true) && destroyedArmour ||
                 !destroyedArmour && !UberEquipment.contains("pickaxe", true))
    ...
    private void changeWeapons() {
        if (!destroyedArmour && !UberEquipment.contains("pickaxe", true)) {
            weapon = UberEquipment.getItem(UberEquipment.WEAPON).getName();
            UberInventory.getItem("pickaxe").interact("wield");
            Game.openTab(Game.TAB_EQUIPMENT);
            sleep(Random.nextInt(500, 600));
            Game.openTab(Game.TAB_INVENTORY);
        }
        else if (UberInventory.getItem(weapon) != null) {
            UberInventory.getItem(weapon).interact("wield");
            Game.openTab(Game.TAB_EQUIPMENT);
            sleep(Random.nextInt(500, 600));
            Game.openTab(Game.TAB_INVENTORY);
        }
    ...
    private void destroyArmour() {
        if (UberInventory.getItem("pickaxe") != null && !UberEquipment.contains("pickaxe"))
            UberInventory.getItem("pickaxe").click(true);
