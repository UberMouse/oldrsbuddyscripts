...
public class SkeletalHorde extends Plugin
{
    ...
    public boolean isValid() {
        return UberNpcs.getNearest(".*skinweaver*") != null && !talkedToSkinWeaver;
    }
    ...
    public int loop() {
        if (MyPlayer.currentRoom().getNearestNpc(".*skinweaver.*") != null &&
            ...
            blockTunnel = true;
        if (MyCombat.canUsePrayer((UberPrayer.isUsingRegularPrayers()) ?
                                  UberPrayer.Prayer.PROTECT_FROM_MISSILES :
                                  UberPrayer.Prayer.DEFLECT_MISSILE)) {
            UberPrayer.Prayer prayer = (UberPrayer.isUsingRegularPrayers()) ?
                                     UberPrayer.Prayer.PROTECT_FROM_MISSILES :
                                     UberPrayer.Prayer.DEFLECT_MISSILE;
    ...
    private void talk() {
        ...
        if (MyPlayer.isMoving() && Calculations.distanceTo(skinweaver) > 3)
            return;
        UberMovement.turnTo(skinweaver);
        ...
        if (!MyPlayer.location().equals(healTile)) {
            ...
            while (MyPlayer.hp() < 80 && MyPlayer.location().equals(healTile)) {
                ...
                if (MyPlayer.interacting() == null && !MyPlayer.inCombat()) {
                    ...
                    UberCamera.turnTo(Enemy.getNPC());
