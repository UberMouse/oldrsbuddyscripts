...
public class ItemHandler extends RSBuddyCommon
{
    ...
    public static boolean isBound(int EquipmentIndex) {
        Item equipItem = UberEquipment.getItem(EquipmentIndex);
    ...
    public static void unBind(int EquipmentIndex) {
        ...
        int timeout = 0;
        UberEquipment.getItem(EquipmentIndex).interact("Remove");
    ...
    public static void bind(int itemID) {
        Item item = UberInventory.getItem(itemID);
        if (item != null) {
            while (!Menu.click("Bind") && !UberInventory.getItem(itemID).getName().contains("(b)")) {
    ...
    public static boolean shouldEquip(Equipable toEquip, int EquipmentIndex) {
        ...
        if (toEquip.getName().contains("(b)"))
            return true;
        else if (toEquip.getEquipmentIndex() == UberEquipment.AMMO)
            return false;
        if (EquipmentIndex == -1) {
            return false;
        }
        if (UberEquipment.getItem(EquipmentIndex).getId() != -1) {
            toCheck = new Equipable(UberEquipment.getItem(EquipmentIndex).getName());
        ...
        if (toEquip.getStyle() == MyPlayer.combatStyle() &&
            ...
            if (UberEquipment.getItem(UberEquipment.WEAPON).getId() != -1) {
                if ((new Equipable(UberEquipment.getItem(UberEquipment.WEAPON).getName()).twoHanded() &&
                     toEquip.getEquipmentIndex() == UberEquipment.SHIELD))
                    return false;
                else if (UberEquipment.getItem(UberEquipment.SHIELD).getId() != -1 && toEquip.twoHanded())
                    return false;
            }
            return true;
