    ...
    public int loop() {
        ...
        for (int[] color : COLOR_TILES) {
            for (GameObject tObj : MyPlayer.currentRoom().getObjects(color)) {
                if (UberNpcs.getTopAt(tObj.getLocation()) == null) {
                    blocksAligned = false;
        ...
        if (blocksAligned) {
           ...
           while(Inventory.containsOneOf(COLOR_VIALS) && !solved) {
                for (int I = 0; I < COLOR_TILES.length; I++) {
                    ...
                    UberNpc block = UberNpcs.getTopAt(safeTile);
                    if (block != null && Inventory.contains(COLOR_VIALS[I])) {
                        UberMovement.turnTo(block);
                        UberInventory.useItem(Inventory.getItem(COLOR_VIALS[I]), block.getNpc());
                        Util.waitToStop();
                    }
                }
                sleep(200, 400);
            ...
            for (int[] color : COLOR_TILES) {
                if (tObj == null || UberNpcs.getTopAt(tObj.getLocation()) != null)
            ...
            if (push != null && UberNpcs.getTopAt(safeTile) == null) {
                ...
                if (checkX1 != null) {
                    xBlocked = false;
                    UberNpc block = UberNpcs.getTopAt(checkX1);
                    if (block == null)
                        UberNpcs.getTopAt(checkX2);
                ...
                if (checkY1 != null) {
                    yBlocked = false;
                    UberNpc block = UberNpcs.getTopAt(checkY1);
                    if (block == null)
                        UberNpcs.getTopAt(checkY2);
