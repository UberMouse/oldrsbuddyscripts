...
public class SuspiciousGrooves extends PuzzlePlugin
{
    ...
    public int loop() {
        ...
        UberMovement.turnTo(groove);
        if (groove.interact("Step-onto")) {
            for (int i = 0; i <= 15 && !UberPlayer.location().equals(groove.getLocation()); i++)
                Task.sleep(100);
            for (int i = 0; i <= 10 && UberPlayer.location().equals(groove.getLocation()); i++)
                Task.sleep(100);
            if (UberPlayer.location().equals(groove.getLocation())) {
