    ...
    public int execute() {
        for (Room room : Explore.getRooms()) {
            if (room.contains(MyPlayer.location())) {
                ...
                //                if (!UberPlayer.lastDoorOpened().isOpen() &&
                //                    Calculations.distanceTo(UberPlayer.lastDoorOpened().getLocation()) < 5) {
                //                    UberPlayer.lastDoorOpened().setOpen(true);
                //                    UberPlayer.lastRoom()
                //                            .updateDoor(UberPlayer.lastDoorOpened());
                //                    UberPlayer.lastDoorOpened().setConnector(UberPlayer.currentRoom());
                //                    Explore.getDoors().remove(UberPlayer.lastDoorOpened());
                //                    Explore.getDoors().add(UberPlayer.lastDoorOpened());
                //                }
