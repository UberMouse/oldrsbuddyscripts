...
public class MyCombat extends RSBuddyCommon
{
    ...
    public static String[] getAttackStyles() {
        return Weapons.toValue(new Equipable(UberEquipment.getItem(UberEquipment.WEAPON).getName()).getItemName())
                      .attackStyles();
    }
    ...
    public static boolean canUsePrayer(UberPrayer.Prayer prayer) {
        if (prayer == null)
            return false;
    ...
    public static void doPrayerFor(EnemyDef enemy) {
        if (!Options.getBoolean("usePrayers"))
            return;
        UberPrayer.Prayer prayer = styleToPrayer(enemy.recommendedPrayer());
     ...
     */
    public static UberPrayer.Prayer styleToPrayer(int style) {
        switch (style) {
            case 0:
                return (UberPrayer.isUsingRegularPrayers()) ?
            ...
            case 2:
                return (UberPrayer.isUsingRegularPrayers()) ?
                       UberPrayer.Prayer.PROTECT_FROM_MAGIC :
                       UberPrayer.Prayer.DEFLECT_MAGIC;
        }
        return null;
    }
}
