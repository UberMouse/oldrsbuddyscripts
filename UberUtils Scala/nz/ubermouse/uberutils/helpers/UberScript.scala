...
import tasks.{ImageThread, PriceThread}
import nz.ubermouse.uberutils.paint.PaintController
import nz.ubermouse.uberutils.paint.abstracts.PComponent
import nz.ubermouse.uberutils.paint.components.{PFancyButton, PCheckBox, PButton, PFrame}
                                  ...
                                  MouseMotionListener
{
   ...
   def onRepaint(graphics: Graphics) {
      ...
      try {
         import nz.ubermouse.uberutils.paint.components.PCheckBoxLayout
         val g: Graphics2D = graphics.asInstanceOf[Graphics2D]
         var clayout: PComponent = null
         try {
            import nz.ubermouse.uberutils.paint.components.PColumnLayout
         ...
         for (skill <- skills) {
            if (skill.xpGained > 0) {
               import nz.ubermouse.uberutils.paint.components.PSkill
